...
class Widget(graphics.Sprite):
    ...
    #: :attr:`y_align` properties to adjust alignment when fill is set
    #: to False.
    fill = True
    ...
    # : Mouse cursor. see :attr:`graphics.Sprite.mouse_cursor` for values
    mouse_cursor = False
    ...
    def __init__(self, width=None, height=None, expand=None, fill=None,
        ...
        self.padding_right = (
            padding_right or
            self.__class__.padding_right or
            self.padding_right or
            0
            )
        self.padding_bottom = (
            padding_bottom or
            self.__class__.padding_bottom or
            self.padding_bottom or
            0
            )
        self.padding_left = (
            padding_left or
            self.__class__.padding_left or
        ...
        self.margin_right = (
            margin_right or
            self.__class__.margin_right or
            self.margin_right or
            0
            )
        self.margin_bottom = (
            margin_bottom or
            self.__class__.margin_bottom or
            self.margin_bottom or
            0
            )
        self.margin_left = (
    ...
    def __setattr__(self, name, val):
        # forward width and height to min_width and min_height as i've ruined
        # the setters a bit i think
        if name == "width":
            name = "min_width"
        elif name == "height":
            name = "min_height"
            ...
            if len(val) == 1:
                self.padding_top = self.padding_right = val[0]
            ...
            if len(val) == 1:
                self.margin_top = self.margin_right = val[0]
        ...
        # in widget case visibility affects placement and everything so request
        # repositioning from parent
        if name == 'visible' and getattr(self, "parent", None) and getattr(
                self.parent, "resize_children", None):
    ...
    def __on_mouse_over(self, sprite):
        cursor, mouse_x, mouse_y, mods = (
            sprite.get_scene().get_window().get_pointer()
            )
    ...
    def queue_resize(self):
        ...
        if parent and isinstance(parent, graphics.Sprite) and hasattr(
                parent, "queue_resize"):
            parent.queue_resize()
    ...
    def get_min_size(self):
        ...
        if self.visible is False:
            ...
            return (
                    (
                        (self.min_width or 0) + self.horizontal_padding +
    ...
    def width(self):
        ...
        if self.parent and isinstance(self.parent, graphics.Scene):
            ...
            if not self._scene_resize_handler:
                ...
                self._scene_resize_handler = self.parent.connect(
                    "on-resize", res)

        min_width = (self.min_width or 0) + \
            self.margin_left + self.margin_right
    ...
    def height(self):
        ...
        min_height = (self.min_height or 0) + \
            self.margin_top + self.margin_bottom
    ...
    def enabled(self):
        ...
        if self.parent and isinstance(self.parent, Widget):
            if self.parent.enabled is False:
                return False
...
def get_min_size(sprite):
    if hasattr(sprite, "get_min_size"):
        ...
        min_width, min_height = getattr(
            sprite, "width", 0), getattr(
            sprite, "height", 0)
...
class Container(Widget):
    """The base container class that all other containers inherit from.
       You can insert any sprite in the container, just make sure that
       it either has width and height defined so that the container can do
       alignment, or for more sophisticated cases, make sure it has
       get_min_size function that returns how much space is needed.
    ...
    def get_min_size(self):
        ...
        if self._cached_w is None:
            ...
            width += (
                self.horizontal_padding + self.margin_left + self.margin_right
                )

            height = max([get_min_size(sprite)[1]
                          for sprite in sprites] or [0])
            height += (
                self.vertical_padding + self.margin_top + self.margin_bottom
                )

            self._cached_w, self._cached_h = max(
                width, self.min_width or 0), max(
                height, self.min_height or 0)
    ...
    def resize_children(self):
        ...
        for sprite, props in (get_props(sprite)
                              for sprite in self.sprites if sprite.visible):
            ...
            h = h * sprite.scale_y + \
                props["margin_top"] + props["margin_bottom"]

            sprite.x = self.padding_left + props["margin_left"] + (
                max(sprite.alloc_w * sprite.scale_x, w) - w) * getattr(
                    sprite, "x_align", 0
                    )
            sprite.y = self.padding_top + props["margin_top"] + (
                max(sprite.alloc_h * sprite.scale_y, h) - h) * getattr(
                    sprite, "y_align", 0
                    )
...
class Bin(Container):
    ...
    def get_height_for_width_size(self):
        ...
        # width = width + self.horizontal_padding + self.margin_left +
        #  self.margin_right
        # height = height + self.vertical_padding + self.margin_top +
        #  self.margin_bottom
...
class Box(Container):
    ...
    #: whether the box is packing children horizontally (from left to right)
        ...
        for sprite in self.sprites:
            if sprite.visible:
                if getattr(sprite, "expand", True):
                    ...
                    if hasattr(sprite, "get_min_size"):
                        size = (
                            sprite.get_min_size()[0] if (
                                self.orient_horizontal
                                ) else sprite.get_min_size()[1]
                            )
                    else:
                        size = getattr(
                            sprite, "width", 0
                            ) * sprite.scale_x if self.orient_horizontal else (
                                getattr(
                                    sprite, "height", 0
                                    ) * sprite.scale_y
                                )
    ...
    def resize_children(self):
        ...
        for sprite, props in sprites:
            if self.orient_horizontal:
                ...
                if hasattr(sprite, "get_height_for_width_size"):
                    size = max(
                        size,
                        sprite.get_height_for_width_size()[1] *
                        sprite.scale_y)
        ...
        if sprite_sizes:
            remaining_space = remaining_space - \
                sum(sprite_sizes) - self.get_total_spacing()

        interested_sprites = [
            sprite for sprite,
            props in sprites if getattr(
                sprite,
                "expand",
                True)]
        ...
        for (sprite, props), min_size in zip(sprites, sprite_sizes):
            ...
            sprite.x = self.padding_left + x_pos + props["margin_left"] + (
                max(sprite.alloc_w * sprite.scale_x, w) - w
                ) * getattr(sprite, "x_align", 0.5)
            sprite.y = self.padding_top + y_pos + props["margin_top"] + (
            ...
            if (min_size + sprite_bonus) > 0:
                if self.orient_horizontal:
                    x_pos += int(max(w, sprite.alloc_w *
                                     sprite.scale_x)) + self.spacing
                else:
                    y_pos += max(h, sprite.alloc_h *
                                 sprite.scale_y) + self.spacing
    ...
    def get_height_for_width_size(self):
        ...
        width = (
            width + self.horizontal_padding + self.margin_left +
            self.margin_right
            )
        height = (
            height + self.vertical_padding + self.margin_top +
            self.margin_bottom
            )
...
class _DisplayLabel(graphics.Label):
    ...
    def __setattr__(self, name, val):
        ...
        if name in ("text", "markup", "size", "wrap",
                    "ellipsize", "max_width"):
            if name != "max_width":
                self._cached_w, self._cached_h = None, None
            self._cached_wh_w, self._cached_wh_h = None, None
...
class Label(Bin):
    ...
    def __init__(self, text="", markup="", spacing=5, image=None,
        ...
        # TODO - am initiating table with fill = false but that yields
        # suboptimal label placement and the 0,0 points to whatever parent gave
        # us
        ...
        # the default is 0.5 which makes label align incorrectly on wrapping
        self.display_label.x_align = 0
        ...
        #: either the pango `wrap <http://www.pygtk.org/pygtk2reference/
        #: pango-constants.html#pango-wrap-mode-constants>`_
        #: or `ellipsize <http://www.pygtk.org/pygtk2reference/
        #: pango-constants.html#pango-ellipsize-mode-constants>`_ constant.
    ...
    def __setattr__(self, name, val):
        if name in ("text", "markup", "color", "size"):
            if self.display_label.__dict__.get(
                    name, "hamster_graphics_no_value_really") is val:
                return
        ...
        if name in ('x_align', 'y_align') and hasattr(self, "container"):
            ...
            name == "alloc_w" and
            hasattr(self, "display_label") and
            getattr(self, "overflow") is not False
        ):
            ...
            if val is False:
                ...
                isinstance(val, pango.WrapMode) and
                val in (
                    pango.WrapMode.WORD, pango.WrapMode.WORD_CHAR,
                    pango.WrapMode.CHAR
                    )
            ):
                ...
                isinstance(val, pango.EllipsizeMode) and
                val in (
        ...
        if name in ("text", "markup", "image",
                    "image_position", "overflow", "size"):
    ...
    def _update_max_width(self):
        ...
        if self.overflow is False:
            ...
            w = (self.alloc_w or 0) - \
                self.horizontal_padding - self.container.spacing
    ...
    def _position_contents(self):
        if self.image and (self.text or self.markup):
            ...
            self.container.orient_horizontal = self.image_position in (
                "left", "right")
