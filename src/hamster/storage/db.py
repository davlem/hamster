...
try:
    ...
    print(
        "Could not import gio - requires pygobject. File monitoring will be "
        "disabled"
        )
    gio = None
...
class Storage(storage.Storage):
    ...
    def __init__(self, unsorted_localized="Unsorted", database_dir=None):
        ...
        if gio:
            ...
            def on_db_file_change(monitor, gio_file, event_uri, event):
                ...
                if event == gio.FileMonitorEvent.CHANGES_DONE_HINT:
                    logger.warning(
                        "DB file has been modified externally. Calling all "
                        "stations"
                        )
    ...
    def __init_db_file(self, database_dir):
        if not database_dir:
            try:
                ...
                if not xdg_data_home:
                    ...
                    logger.warning(
                        "No xdg_data_home - assuming ~/.local/share")
        ...
        if not os.path.exists(db_path):
            ...
            old_db_path = os.path.join(
                xdg_data_home, 'hamster-applet', 'hamster.db')
            if os.path.exists(old_db_path):
                ...
                try:
                    ...
                    data_dir = os.path.join(
                        defs.DATA_DIR, "hamster-time-tracker")
                    ...
                    if os.path.exists(os.path.join(module_dir, "data")):
                        ...
                        # get ./data from ./src/hamster/storage/db.py (3 levels
                        # up)
                        data_dir = os.path.join(
                            module_dir, '..', '..', '..', 'data')
                logger.warning(
                    (
                        "Database not found in {} - installing default "
                        "from {}!"
    ...
    def __get_tags(self, only_autocomplete=False):
        if only_autocomplete:
            return self.fetchall(
                "select * from tags where autocomplete != 'false' "
                "order by name"
                )
        else:
    ...
    def __get_tag_ids(self, tags):
        ...
        db_tags = self.fetchall(
            "select * from tags where name in (%s)" % ",".join(
                ["?"] * len(tags)
                ),
            tags
        ...
        set_complete = [str(tag["id"])
                        for tag in db_tags if tag["autocomplete"] == "false"]
        if set_complete:
            changes = True
            self.execute(
    ...
    def __update_autocomplete_tags(self, tags):
        ...
        to_uncomplete = [str(tag["id"]) for tag in gone if tag["occurences"]
                         > 0 and tag["autocomplete"] == "true"]
    ...
    def __update_activity(self, id, name, category_id):
        ...
        affected_ids = [res[0] for res in self.fetchall(
    ...
    def __change_category(self, id, category_id):
        ...
        existing_activity = self.__get_activity_by_name(
            activity['name'], category_id)

        # we are already there, go home
        if existing_activity and id == existing_activity['id']:
            return False

        if existing_activity:  # ooh, we have something here!
            # first move all facts that belong to movable activity to the new
            # one
            update = """
                       UPDATE facts
                          SET activity_id = ?
        ...
        if existing_activity:
            affected_ids.extend([res[0] for res in self.fetchall(
    ...
    def __update_category(self, id, name):
        if id > -1:
            # Update, and ignore unsorted, if that was somehow triggered
            update = """
                       UPDATE categories
        ...
        affected_query = """
            SELECT id FROM facts WHERE activity_id in (
                SELECT id FROM activities where category_id=?)
        """
    ...
    def __get_activity_by_name(self, name, category_id=None, resurrect=True):
        ...
        if category_id:
            ...
            res = self.fetchone(
                query, (self._unsorted_localized, name, category_id))
        else:
            query = """
               SELECT a.id, a.name, a.deleted, coalesce(b.name, ?) as category
                 FROM activities a
    ...
    def __group_tags(self, facts):
        ...
        for fact_id, fact_tags in itertools.groupby(facts, lambda f: f["id"]):
            ...
            # in python 2.5, sqlite does not have keys() yet, so we hardcode
            # them (yay!)
    ...
    def __squeeze_in(self, start_time):
        ...
        # in the latter case go only few hours ahead. everything else is
        # madness, heh
        query = """
               SELECT a.*, b.name
                 FROM facts a
            LEFT JOIN activities b on b.id = a.activity_id
                WHERE (
                    ...
                    (start_time > ? and start_time < ? and end_time is null) OR
    ...
    def __solve_overlaps(self, start_time, end_time):
        ...
        for fact in conflicts:
            ...
            # won't eliminate as it is better to have overlapping entries than
            # loosing data
            if start_time < fact["start_time"] and end_time > fact_end_time:
                continue

            # split - truncate until beginning of new entry and create new
            # activity for end
            if fact["start_time"] < start_time < fact_end_time and \
               fact["start_time"] < end_time < fact_end_time:
                ...
                new_fact_id = self.__add_fact(
                    new_fact.serialized_name(), end_time, fact_end_time)
                ...
                tag_update = """INSERT INTO fact_tags(fact_id, tag_id)
                                     SELECT ?, tag_id
                                      ...
                                      WHERE fact_id = ?"""
                self.execute(
                    tag_update, (new_fact_id, fact["id"]))  # clone tags
    ...
    def __add_fact(self, serialized_fact, start_time,
                   end_time=None, temporary=False):
        ...
        if (dt.timedelta(days=-1) <= hamster_now() -
            ...
            if previous and previous['start_time'] <= start_time:
                ...
                # restart
                if (
                    previous["activity_id"] == activity_id and
                    set(previous["tags"]) == set([tag[1] for tag in tags]) and
                ...
                if (
                    not previous["description"] and
                    60 >= (start_time - previous['start_time']).seconds >= 0
                ):
                    self.__remove_fact(previous['id'])

                    # now that we removed the previous one, see if maybe the
                    # one before that is actually same as the one we want
                    # to start (glueing)
                    if (
                        len(facts) > 1 and
                        60 >= (start_time - facts[-2]['end_time']).seconds >= 0
                    ):
                        before = facts[-2]
                        if (
                            before["activity_id"] == activity_id and
                            set(before["tags"]) == set(
        ...
        # now link tags
        insert = [
            "insert into fact_tags(fact_id, tag_id) values(?, ?)"] * len(tags)
    ...
    def __get_facts(self, date, end_date=None, search_terms=""):
        ...
        datetime_to = dt.datetime.combine(
            end_date, split_time) + dt.timedelta(days=1, seconds=-1)

        logger.info(
            "searching for facts from {} to {}".format(
        ...
        query = """
                ...
                LEFT JOIN tags e ON e.id = d.tag_id
                    WHERE (
                        a.end_time >= ? OR a.end_time IS NULL
                        ) AND a.start_time <= ?
        """
        ...
        if search_terms:
            ...
            search_terms = search_terms.replace(
                ...
                "''")
            query += """ AND a.id %s IN (
                SELECT id FROM fact_index WHERE fact_index MATCH '%s')""" % (
        ...
        for fact in facts:
            ...
            if fact["end_time"]:
                ...
                (hamster_today() == fact["start_time"].date()) or
            ...
            fact_start_date = fact["start_time"].date() \
                - dt.timedelta(1 if fact["start_time"].time()
                               < split_time else 0)
    ...
    def __remove_activity(self, id):
        ...
        if bound_facts > 0:
            self.execute(
                "UPDATE activities SET deleted = 1 WHERE id = ?", (id,))
        else:
    ...
    def __check_index(self, start_date, end_date):
        ...
        rebuild_ids = ",".join([
            str(res[0]) for res in self.fetchall(
                index_query,
                (start_date, end_date)
                )
            ])
        ...
        if rebuild_ids:
            ...
            facts = self.__group_tags(self.fetchall(
                query, (self._unsorted_localized, )))
    ...
    def execute(self, statement, params=()):
        ...
        if isinstance(statement, list) is False:
    ...
    def run_fixtures(self):
        ...
        if version < 8:
            ...
            for activity in activities:
                self.execute(
                    statement, (activity['name'].lower(), activity['id']))
            ...
            for category in categories:
                self.execute(
                    statement, (category['name'].lower(), category['id']))
        ...
        if version < current_version:
            ...
            print(
                "updated database from version %d to %d" %
