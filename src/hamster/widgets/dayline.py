    ...
    def on_render(self, sprite):
        ...
        if self.end_label.visible:
            ...
            if self.duration_label.visible:
                ...
                self.duration_label.x = (
                    self.width - self.duration_label.width) / 2
        else:
...
class DayLine(graphics.Scene):
    ...
    def plot(self, date, facts, select_start, select_end=None):
        ...
        for fact in facts:
            ...
            fact_bar = graphics.Rectangle(0, 0, fill="#aaa", stroke="#aaa")
            fact_bar.fact = fact
    ...
    def on_enter_frame(self, scene, context):
        ...
        for bar in self.fact_bars:
            ...
            minutes = bar_start_time.seconds / 60 + \
            ...
            if self.chosen_selection.end_time:
                self.chosen_selection.width = round(
                    (self.chosen_selection.end_time -
                     self.chosen_selection.start_time).seconds /
                    60 /
                    minute_pixel)
            else:
        ...
        # current time
        if self.view_time < stuff.hamster_now() < self.view_time + \
                dt.timedelta(hours=self.scope_hours):
            minutes = round(
                (stuff.hamster_now() -
                 self.view_time).seconds /
                60 /
                minute_pixel)
